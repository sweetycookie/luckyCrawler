...
def job_lucky() :
  ...
  last_round = get_data[0]
  old_last_round = lucky_soup.select_one('.win_result h4 > strong').text

  if last_round != old_last_round :
...
def job_pension() :
  ...
  last_round = get_data[0]
  old_last_round = pension_soup.select_one('#after720 h4 > strong').text
